package com.medgo.claims.components;
...
public class MedGoCustomSecurityContext {
    ...
    @PostConstruct
    public void init() {
        log.info("MedGo MedGoCustomSecurityContext initialized for claims-service");
    }
    ...
    public String authenticatedUsername(HttpServletRequest request) {
        log.debug("@authenticatedUsername with HttpServletRequest");
    ...
    public String authenticatedMemberCode(HttpServletRequest request) {
        log.debug("@authenticatedMemberCode with HttpServletRequest");
        String token = getTokenFromRequest(request);
        if (token != null) {
            try {
                return jwtTokenUtil.getMemberCodeFromTokenClaim(token);
